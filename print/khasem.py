...
def RIEMP_Print(file_path, account_number, report_date, table_data):
   ...
   # left_align_header = ParagraphStyle( name='LeftAlign',fontName='Amiri', fontSize=12, alignment=2)  # 1 corresponds to left alignment
    ...
    table_headers = [
        reshape_arabic("رقم الحساب"),
        reshape_arabic("المجموع"),
        reshape_arabic("منحة العمرة"),
        reshape_arabic("منحة التقاعد"),
        reshape_arabic("ادراج مبلغ"),
        reshape_arabic("منحة للمواليد"),
        reshape_arabic("منحة للختان"),
        reshape_arabic("منحة وفاة"),
        reshape_arabic("منحة مرضية"),
    ...
    # Calculate column widths - optimized to equal exactly 100% of landscape width
    available_width = width - 15  # Total width minus margins
    col_widths = [
        available_width * 0.15,  # رقم الحساب (11%)
        available_width * 0.07,  # المجموع (7%)
        available_width * 0.07,  # منحة العمرة (7%)
        available_width * 0.07,  # منحة التقاعد (7%)
        available_width * 0.07,  # ادراج مبلغ (7%)
        available_width * 0.07,  # منحة للمواليد (7%)
        available_width * 0.07,  # منحة للختان (7%)
        available_width * 0.07,  # منحة وفاة (7%)
        available_width * 0.07,  # منحة مرضية (7%)
        available_width * 0.06,  # المبلغ (6%)
        available_width * 0.10,  # مكان العمل (10%)
        available_width * 0.10,  # الإسم و اللقب (13%)
        available_width * 0.04   # الرقم (4%)
    ]
